/*************************************************************************
   ...
   V2.0	11/01/2023
      - modified KK4DAS, Dean Souleles version to work on the RPi Pico.
	  - need to modify DEBUG functions so please don't use at this time.

***************************************************************************/

#include "Arduino.h"
#include "pico_IC746.h"
...
/*
   Simple constructor that initiates the CAT serial port as Serial2 and the
   chosen baudrate in the radio.begin(specify baudrate) located in the setup() function
*/

void IC746::begin(long br) {
  // Configure Serial2 TX/RX pins on RPi Pico
  Serial2.setTX(8);
  Serial2.setRX(9);
  // Start Serial2 instance on RPi Pico
  Serial2.begin(br);
  Serial2.flush();
}
...
void IC746::send(byte *buf, int len) {
  ...
  Serial2.write(CAT_PREAMBLE);
  Serial2.write(CAT_PREAMBLE);

  for (i = 0; i < len; i++) {
    Serial2.write(buf[i]);
  }
  Serial2.write(CAT_EOM);
...
boolean IC746::readCmd() {
  ...
  while (Serial2.available() && !cmdRcvd) {

    bt = byte(Serial2.read());
